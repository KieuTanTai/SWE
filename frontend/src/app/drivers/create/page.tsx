// src/app/drivers/create/page.tsx
"use client";
...
export default function CreateDriverPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    // Driver Info
    driver_experience: 0,
    driver_experience_type: "year", // Default
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        // Validate cơ bản
        if(!formData.person_name) throw new Error("Full Name is required");

        // Gọi Service tạo mới
        await driverService.createDriver({
            ...formData,
            driver_experience: Number(formData.driver_experience),
            person_type: 'driver' // Backend cần trường này
        });

        alert("Driver created successfully!");
        router.push("/drivers"); // Quay về trang danh sách
    } catch (error) {
        console.error(error);
    ...
    <div className="p-6 max-w-3xl mx-auto text-gray-100">
      ...
      <div className="bg-gray-800 rounded-lg shadow-xl border border-gray-700 p-8">
        <h1 className="text-2xl font-bold text-white mb-6 flex items-center gap-2 pb-4 border-b border-gray-700">
          <Car className="text-blue-500" /> Add New Driver
        </h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="person_name" value={formData.person_name} onChange={handleChange} required placeholder="John Doe" className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-300 mb-2">Phone Number</label>
                        <input type="tel" name="person_phone" value={formData.person_phone} onChange={handleChange} placeholder="090xxxxxxx" className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-300 mb-2">Gender</label>
                        <select name="person_gender" value={formData.person_gender} onChange={handleChange} className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-500 focus:outline-none">
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" name="driver_experience" value={formData.driver_experience} onChange={handleChange} min="0" required className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                    </div>
                    <div>
            ...
            <div className="pt-4 flex justify-end">
                <button type="submit" disabled={loading} className="flex items-center justify-center px-6 py-3 rounded-lg font-medium text-white bg-blue-600 hover:bg-blue-700 shadow-lg transition-all disabled:opacity-50">
                    {loading ? "Saving..." : <><Save size={20} className="mr-2" /> Create Driver</>}
                </button>
            </div>
        </form>
      </div>
