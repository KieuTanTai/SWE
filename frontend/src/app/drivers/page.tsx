...
export default function DriverAndBusPage() {
  ...
  const filteredDrivers = drivers.filter(d => {
      ...
      const name = d.person?.person_full_name?.toLowerCase() || "";
                        ...
                        {filteredDrivers.length > 0 ? filteredDrivers.map((driver) => {
                            ...
                            return (
                                <tr key={driver.driver_person_id} className={`hover:bg-gray-750 transition-colors ${!isActive ? 'opacity-50 bg-gray-900' : ''}`}>
                                    ...
                                    <td className="px-6 py-4"><div className="flex items-center"><div className="h-8 w-8 rounded-full bg-blue-900/30 text-blue-400 flex items-center justify-center mr-3"><User size={16}/></div><div><div className="text-sm font-medium text-white">{driver.person?.person_full_name || "Unknown"}</div><div className="text-xs text-gray-500">{driver.person?.person_email}</div></div></div></td>
